"""
Script for monitoring hypernodes. Can be run from any directory and as cron job
Creates three files defined in hn_config.py:
* OUTFILE_1 (status.json)     : ip address and current block_height
* OUTFILE_2 (hypernodes.json) : comprehensive data in 3 vars (heights, status and data)
* OUTFILE_3 (status_ex.json)  : similar to OUTFILE_2, different formatting
"""
...
if __name__ == "__main__":
    ...
    for i in range(0,L):
        ...
        status_ex[i].append(H2)
        status_ex[i].append(ver)
        if H2 > 0:
            status_ex[i].append("Active")
        else:
            status_ex[i].append("Inactive")
